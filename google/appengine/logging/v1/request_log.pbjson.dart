///
//  Generated code. Do not modify.
//  source: google/appengine/logging/v1/request_log.proto
//
// @dart = 2.12
// ignore_for_file: annotate_overrides,camel_case_types,unnecessary_const,non_constant_identifier_names,library_prefixes,unused_import,unused_shown_name,return_of_invalid_type,unnecessary_this,prefer_final_fields,deprecated_member_use_from_same_package

import 'dart:core' as $core;
import 'dart:convert' as $convert;
import 'dart:typed_data' as $typed_data;
@$core.Deprecated('Use logLineDescriptor instead')
const LogLine$json = const {
  '1': 'LogLine',
  '2': const [
    const {'1': 'time', '3': 1, '4': 1, '5': 11, '6': '.google.protobuf.Timestamp', '10': 'time'},
    const {'1': 'severity', '3': 2, '4': 1, '5': 14, '6': '.google.logging.type.LogSeverity', '10': 'severity'},
    const {'1': 'log_message', '3': 3, '4': 1, '5': 9, '10': 'logMessage'},
    const {'1': 'source_location', '3': 4, '4': 1, '5': 11, '6': '.google.appengine.logging.v1.SourceLocation', '10': 'sourceLocation'},
  ],
};

/// Descriptor for `LogLine`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List logLineDescriptor = $convert.base64Decode('');
@$core.Deprecated('Use sourceLocationDescriptor instead')
const SourceLocation$json = const {
  '1': 'SourceLocation',
  '2': const [
    const {'1': 'file', '3': 1, '4': 1, '5': 9, '10': 'file'},
    const {'1': 'line', '3': 2, '4': 1, '5': 3, '10': 'line'},
    const {'1': 'function_name', '3': 3, '4': 1, '5': 9, '10': 'functionName'},
  ],
};

/// Descriptor for `SourceLocation`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List sourceLocationDescriptor = $convert.base64Decode('Cg5Tb3VyY2VMb2NhdGlvbhISCgRmaWxlGAEgASgJUgRmaWxlEhIKBGxpbmUYAiABKANSBGxpbmUSIwoNZnVuY3Rpb25fbmFtZRgDIAEoCVIMZnVuY3Rpb25OYW1l');
@$core.Deprecated('Use sourceReferenceDescriptor instead')
const SourceReference$json = const {
  '1': 'SourceReference',
  '2': const [
    const {'1': 'repository', '3': 1, '4': 1, '5': 9, '10': 'repository'},
    const {'1': 'revision_id', '3': 2, '4': 1, '5': 9, '10': 'revisionId'},
  ],
};

/// Descriptor for `SourceReference`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List sourceReferenceDescriptor = $convert.base64Decode('Cg9Tb3VyY2VSZWZlcmVuY2USHgoKcmVwb3NpdG9yeRgBIAEoCVIKcmVwb3NpdG9yeRIfCgtyZXZpc2lvbl9pZBgCIAEoCVIKcmV2aXNpb25JZA==');
@$core.Deprecated('Use requestLogDescriptor instead')
const RequestLog$json = const {
  '1': 'RequestLog',
  '2': const [
    const {'1': 'app_id', '3': 1, '4': 1, '5': 9, '10': 'appId'},
    const {'1': 'module_id', '3': 37, '4': 1, '5': 9, '10': 'moduleId'},
    const {'1': 'version_id', '3': 2, '4': 1, '5': 9, '10': 'versionId'},
    const {'1': 'request_id', '3': 3, '4': 1, '5': 9, '10': 'requestId'},
    const {'1': 'ip', '3': 4, '4': 1, '5': 9, '10': 'ip'},
    const {'1': 'start_time', '3': 6, '4': 1, '5': 11, '6': '.google.protobuf.Timestamp', '10': 'startTime'},
    const {'1': 'end_time', '3': 7, '4': 1, '5': 11, '6': '.google.protobuf.Timestamp', '10': 'endTime'},
    const {'1': 'latency', '3': 8, '4': 1, '5': 11, '6': '.google.protobuf.Duration', '10': 'latency'},
    const {'1': 'mega_cycles', '3': 9, '4': 1, '5': 3, '10': 'megaCycles'},
    const {'1': 'method', '3': 10, '4': 1, '5': 9, '10': 'method'},
    const {'1': 'resource', '3': 11, '4': 1, '5': 9, '10': 'resource'},
    const {'1': 'http_version', '3': 12, '4': 1, '5': 9, '10': 'httpVersion'},
    const {'1': 'status', '3': 13, '4': 1, '5': 5, '10': 'status'},
    const {'1': 'response_size', '3': 14, '4': 1, '5': 3, '10': 'responseSize'},
    const {'1': 'referrer', '3': 15, '4': 1, '5': 9, '10': 'referrer'},
    const {'1': 'user_agent', '3': 16, '4': 1, '5': 9, '10': 'userAgent'},
    const {'1': 'nickname', '3': 40, '4': 1, '5': 9, '10': 'nickname'},
    const {'1': 'url_map_entry', '3': 17, '4': 1, '5': 9, '10': 'urlMapEntry'},
    const {'1': 'host', '3': 20, '4': 1, '5': 9, '10': 'host'},
    const {'1': 'cost', '3': 21, '4': 1, '5': 1, '10': 'cost'},
    const {'1': 'task_queue_name', '3': 22, '4': 1, '5': 9, '10': 'taskQueueName'},
    const {'1': 'task_name', '3': 23, '4': 1, '5': 9, '10': 'taskName'},
    const {'1': 'was_loading_request', '3': 24, '4': 1, '5': 8, '10': 'wasLoadingRequest'},
    const {'1': 'pending_time', '3': 25, '4': 1, '5': 11, '6': '.google.protobuf.Duration', '10': 'pendingTime'},
    const {'1': 'instance_index', '3': 26, '4': 1, '5': 5, '10': 'instanceIndex'},
    const {'1': 'finished', '3': 27, '4': 1, '5': 8, '10': 'finished'},
    const {'1': 'first', '3': 42, '4': 1, '5': 8, '10': 'first'},
    const {'1': 'instance_id', '3': 28, '4': 1, '5': 9, '10': 'instanceId'},
    const {'1': 'line', '3': 29, '4': 3, '5': 11, '6': '.google.appengine.logging.v1.LogLine', '10': 'line'},
    const {'1': 'app_engine_release', '3': 38, '4': 1, '5': 9, '10': 'appEngineRelease'},
    const {'1': 'trace_id', '3': 39, '4': 1, '5': 9, '10': 'traceId'},
    const {'1': 'trace_sampled', '3': 43, '4': 1, '5': 8, '10': 'traceSampled'},
    const {'1': 'source_reference', '3': 41, '4': 3, '5': 11, '6': '.google.appengine.logging.v1.SourceReference', '10': 'sourceReference'},
  ],
};

/// Descriptor for `RequestLog`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List requestLogDescriptor = $convert.base64Decode('');
